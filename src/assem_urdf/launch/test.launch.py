...
def generate_launch_description():
    ...
    nav2_bringup_directory = get_package_share_directory("nav2_bringup")
    ...
    nav2_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(nav2_bringup_directory, 'launch', 'bringup_launch.py')
        ),
        launch_arguments={
            'use_sim_time': use_sim_time,
            'params_file': nav_params_file,
            'map': '',  # No need to load map, SLAM will create it
